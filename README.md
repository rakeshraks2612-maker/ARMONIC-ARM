...
Results
Benchmarked on AWS EC2 c7g.xlarge (Graviton3, Neoverse V1) running Ubuntu 22.04 Arm64 with Arm Performix (APX) hardware performance counters. Measurements averaged over 5 independent runs.
Table
Metric	Baseline (avg of 5 runs)	Optimized (avg of 5 runs)	Improvement
Wall time	~17.3s	~0.22s	~98.7%
Bottleneck Score (B_s)	~17.3	~0.23	~98.7%
APX samples	23–24	23–24	Validated
Note: Results may vary slightly between runs due to cloud instance scheduling noise (±3%), but the relative improvement remains consistent at ~98.7% across all trials.
The ~98.7% speedup comes from eliminating Python interpreter overhead: Numba compiles the hot loop to native Arm64 machine code with NEON SIMD vectorization on Neoverse V1, bypassing the GIL entirely.
...
Telemetry pipeline	Arm Performix collects hardware counters via apx trace; the Armonic MCP Telemetry Bridge exposes schema-validated telemetry over JSON-RPC 2.0
...
ai_inference	~17.3	~0.23	@njit(fastmath=True)	98.7%
...
Note on B_s: Bottleneck Score is a unitless weighted composite of hardware counters. Absolute scale varies by workload (e.g., single-loop dominance in ai_inference produces smaller numbers than multi-function matmul). The improvement percentage is the normalized metric across all workloads.
Quick Start
Prerequisites
Python 3.10+
...
[ARMONIC] Bottleneck Score: ~17.3 (Memory-bound)
...
[ARMONIC] Optimized Score: ~0.23 | Improvement: ~98.7%
Safety & Validation
...
Inference Server Speed	Reduces Python runtime overhead in inference pipelines (JSON serialization, hot loops)	Benchmark: ai_inference ~17.3s → ~0.22s on Graviton3
...
Demo
2.5-minute demo of autonomous optimization on AWS Graviton Arm64: YouTube
License
MIT — see LICENSE
